...
- **Suite, livrée en [28] le 30/07/2026 : la primitive a quitté ce fichier.** `gate__collect` est devenue `proc_collect`, dans `.claude/lib/proc.sh` — ni `gate.sh` ni `session.sh` ne pouvaient l'héberger sans se rendre dépendants l'un de l'autre. Trois choses à savoir en relisant ce ticket : le corps de la fonction est inchangé sauf qu'elle **rend le code de sortie** au lieu de 0 (le gate le jette, `session_spawn` en a besoin, donc `gate__await` appelle `proc_collect … || true`) ; les trois entrées de mutation « 25 … » ci-dessus visent maintenant `.claude/lib/proc.sh`, et l'entrée de terminaison vise `test/proc.bats` où son test a migré ; et la conclusion « le stop n'a pas de délai à lui » a été reprise telle quelle pour la session, ce qui la rend cohérente sur les deux fenêtres. [28] a aussi sondé ce que ni ce ticket ni lui n'avaient regardé : un vrai Ctrl-C envoie SIGINT au groupe entier et pas au run, et ça tient — pour une raison qui appartient à bash 3.2 (un enfant asynchrone d'un shell non interactif a SIGINT sur `SIG_IGN`) et non au pack.
