...
- **Trou qui reste ici** : le filet borne le **contexte** d'une session, pas son **temps**. [07] a posé un chien de garde sur les branches du gate (`GATE_TIMEOUT`), mais une session `claude` qui pend sans émettre de token n'est bornée par rien — `monitor_watch` ne tique que sur ce que le flux écrit. Un timeout de session est le pendant manquant. **Propriétaire depuis le 29/07/2026 : [23]**, ouvert exprès plutôt que rouvrir ce ticket — la terminaison veut une classe dans `failures_classify`, donc une write-surface plus large que celle-ci. Le mécanisme, lui, reste ici : le compteur de ticks sans nouvelle donnée se lit sur le descripteur ouvert de `monitor_watch`, et la partielle de ligne compte comme « la session a écrit ».
...
- **Vérifié contre le vrai binaire par [20], le 29/07/2026 — et le fake s'était arrêté un cran trop tôt.** Deux résultats. (1) Le bon : sondé contre une capture réelle, `monitor_context_tokens` somme correctement la forme réelle de `message.usage` — `9 + 6495 + 17900 + 3 = 24407`, imbrication `cache_creation` non double-comptée. Ce filet n'a donc pas été conçu contre une fiction, c'était le doute à lever. (2) Le mauvais : **les événements `assistant` du flux par défaut du shim ne portaient aucun `usage`**. Le `tokens=1200` que ce ticket journalisait venait de la dernière ligne `result` — un pic que le moniteur ne peut lire qu'une fois la session terminée. Le flux par défaut exerçait l'analyse, jamais le moment ; seuls les fakes scriptés de `smart-zone.bats` couvraient la terminaison à chaud. Corrigé : les deux événements `assistant` du fake portent `usage`, comme le vrai.
- **Faiblesse structurelle du comptage, maintenant gardée par le contrat.** `monitor__int` ne distingue pas une clé *renommée* d'une clé *absente* : la somme sort simplement plus petite, sans erreur. Une release qui renommerait `cache_read_input_tokens` ferait sous-compter la fenêtre des ~18K du prompt système en cache — le filet tirerait tard, ou jamais — et **aucun test de ce dépôt ne bougerait**, tous utilisant des chiffres synthétiques à eux. Rien à corriger ici (sans schéma, absent et renommé sont indistinguables) : `test/contract-claude.bats` vérifie désormais la présence des quatre clés, une par une, sur chaque événement `assistant` du fake et du vrai binaire.
