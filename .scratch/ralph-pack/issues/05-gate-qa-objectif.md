...
- **Deux tests de ce ticket étaient vacuous, révélés le 28/07/2026 en réparant le gate de mutation.** Les deux entrées correspondantes étaient `BROKEN` (le remplacement perl cassait le fichier au lieu de retirer la garantie), donc elles rougissaient pour la mauvaise raison et masquaient les défauts en dessous :

  1. **`the objective checks run concurrently` ne prouvait pas la concurrence.** Le rendez-vous des deux branches est matérialisé par des marqueurs sous `$RALPH_SHIM_STATE`, qui vivent pour tout le run. En séquentiel, la première tentative échoue — mais la **politique de retry de [07]** en lance une seconde, qui trouve les deux marqueurs déjà posés et passe, sans que les branches aient jamais tourné ensemble. Une garantie d'[05] défaite par un ticket ultérieur, dans un test que personne n'avait de raison de relire. Refermé par `assert_equal "$(claude_call_count)" "1"` : une seule session, donc le rendez-vous a bien été tenu du premier coup.
  2. **`a scope-guard that cannot read the tree` : la garantie est portée deux fois**, et pas par la ligne qu'on croyait. Blanchir `[ -n "$base" ] && [ -n "$now" ] || return 1` laisse le test vert, parce que `pipefail` plus un git qui refuse un argument de tree vide font échouer le pipeline de toute façon. Aucune mutation ne peut isoler la ligne tant que le second mécanisme tient ; l'entrée vise donc la **fonction** (`gate_changed_files` rend 0 sans rien dire) et nomme la garantie plutôt que la ligne. La garde explicite reste : elle dit l'intention, et elle est ce qui tiendrait si `pipefail` disparaissait.

- **Deux failles trouvées par la passe transversale du 29/07/2026, ouvertes en [24] et [25].** Toutes deux dans le gate livré ici, aucune visible depuis un test dont les branches finissent vite.

  1. **Le scope-guard est aveugle à tout ce que `.gitignore` couvre** ([24]). `gate_tree_snapshot` sans argument fait `git add -A` *sans* `--force`, et c'est justifié dans son propre commentaire — tirer le build output d'un projet dans le snapshot ferait ressembler chaque itération à un débordement. La conséquence non dite : une session qui écrit un fichier ignoré hors de sa write-surface obtient `scope=green`. Sondé, avec un `.gitignore` portant `cache/` : `tests=green typecheck=green scope=green`, ticket `resolved`, `cache/payload` toujours là. Le corollaire de `docs/frontiere-de-confiance.md` s'applique mot pour mot — « un contrôle qui exclut une zone doit dire ce qui garde cette zone » — et personne ne garde celle-là.
  2. **Un TERM pendant le fan court-circuite la collecte des verdicts** ([25]). `wait` rend la main immédiatement quand un signal piégé arrive, le `|| true` avale le code > 128, et `gate_run` conclut « pas de verdict » sur des branches encore vivantes. « Le vert doit être mérité » est bien tenu ; ce ticket-ci n'avait pas prévu qu'un **rouge** puisse ne pas l'être.
