...
- **Passe transversale du 06/08/2026 : la liste du classifieur n'a pas été relue contre son critère.** Le critère écrit ici est le bon — la question « budget ? » est posée devant les issues où **rien n'a été jugé** — et la liste qui l'implémente (`failed`, `nothing-delivered`) ne connaît que la session de livraison. Une itération vaut `1 + n` sessions ([06]) : une lentille refusée par l'API n'a rien regardé, ce qui satisfait le critère, mais elle produit un verdict manquant, donc `gate-red`, donc la classe que ce ticket refuse délibérément de pardonner. Sondé avec le même `rate_limit_event` des deux côtés : `budget-pause` et ticket rendu sans retry d'un côté, `Failures: 3` et escalade `failed-impl` de l'autre. C'est [43], et c'est [31] appliqué au budget. Deuxième moitié : `budget_stream_posture` n'est appelé que sur le flux de livraison, donc un run dont les lentilles se font refuser ne pause jamais — il brûle `RETRY_N × (1 + n)` sessions par ticket contre un mur qu'il ne voit pas.

- **Levé le 07/08/2026 par [43], et la liste est maintenant écrite comme la lecture d'un critère.** Le classifieur gagne un bras `gate-red)` gardé par `RALPH_GATE_QUOTA_ONLY`, que le gate ne pose que si **toutes** ses branches rouges sont des lentilles que l'API n'a pas laissées démarrer et si l'arbre confié aux lentilles a pu être remis. Un gate rouge reste donc rouge, et un rouge mérité par une lentille qui a répondu `fail` n'est jamais annulé par une sœur refusée à côté d'elle. Le commentaire au-dessus du `case` a été réécrit autour du critère (« rien n'a été jugé ») et non plus autour des deux outcomes qui existaient à sa naissance — c'est le point du ticket, plus que le bras lui-même.

  Trois choses que ce ticket-ci hérite et qu'il faut savoir avant d'y toucher :

  - **La classe `budget` est la seule de `failures_classify` qui nomme une raison et pas un genre de session.** Elle arrive maintenant de trois outcomes (`failed`, `nothing-delivered`, `gate-red` quota-seul), et deux d'entre eux viennent d'un gate qui a **tourné**. La table de `failures_handle` qui dit qui a déjà remis les règles d'ignore était donc fausse depuis ce ticket-ci pour la combinaison `nothing-delivered` + budget : elle redemandait la remise, réannonçait le fichier d'excludes global et le réinscrivait dans le registre de [41]. Corrigé en demandant le fait (`RALPH_GATE_FRONTIER_READ`, posé par `gate_run`) et non le nom de la classe. **Toute quatrième issue ajoutée devant ce classifieur doit se poser la même question : est-ce qu'un gate a tourné sur ce chemin ?**
  - **La posture du pilote peut désormais venir de deux flux.** `$slot/posture` est écrit depuis le flux de livraison avant le gate, puis écrasé par la posture d'une lentille refusée — **jamais** par-dessus une posture qui dit déjà refusé, parce que celle-là est la mesure que le run a prise en premier. Un lecteur de `$slot/posture` ne peut plus supposer qu'il lit la session de livraison.
