...
- **[46], le 29/08/2026 : il y a un quatrième site, et il est en amont des trois.** La question que ce ticket a posée — *sur quels chemins la remise tombe* — avait une réponse incomplète dans une autre direction que la classe d'échec : **où dans l'itération**. Les trois sites d'ici sont tous derrière `failures_protect_tracker`, et ce garde lit et écrit `issues/` *à travers git* (`failures_tracker_tree` stage le répertoire, `checkout-index` le ressort). Une session qui avait installé un `filter.<n>.clean`/`.smudge` faisait donc réécrire tous les tickets sur le disque, durablement, avant qu'aucun contrôle n'ait regardé la configuration. `loop__iterate` appelle maintenant `gate_frontier_put_back` dès le retour de la session (après le garde d'orphelin de [44]) ; les trois sites d'ici sont inchangés et lisent le mouvement dans le registre de [41] au lieu de le redétecter.

- **Le drapeau `RALPH_GATE_FRONTIER_READ` de [43] reste, et il n'est plus le seul rempart contre la double facture.** Avec un quatrième appelant, « un seul appelant par itération » n'était plus tenable comme convention. `gate__frontier_record` refuse maintenant d'ajouter au registre une ligne que **ce pin** a déjà enregistrée et **pas encore lue** — donc la règle vit dans le registre et non dans la discipline des sites d'appel. La portée « pas encore lue » est ce qui préserve le cas de ce ticket : après que `gate_run` a lu sa part, la marque est passée devant, donc un mouvement de la session de planification du re-slice est un événement neuf et reste facturé.
