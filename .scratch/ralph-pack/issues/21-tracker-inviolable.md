...
- **Contrainte posée par [33], livré le 04/08/2026 : le snapshot du tracker ne dit pas encore quel répertoire il protège.** `failures_tracker_snapshot` appelle `gate_tree_snapshot "$(failures__issues_path)"`, c'est-à-dire la branche à pathspecs — la seule que [33] n'a pas convertie. Un pathspec git est un motif : un `FEATURE` portant un métacaractère ferait porter la protection sur un autre répertoire que celui du run. La branche sans pathspec passe désormais des pathspecs `:(literal)` pour cette raison exacte ; celle-ci prend `"$@"`, donc la conversion demande une boucle et un choix — est-ce que ce chemin-là est un chemin (littéral) ou une surface (glob) ? La réponse est « un chemin », comme pour les chemins gardés. Non fait dans [33] parce qu'il s'agit du tracker et pas de la zone ignorée, et parce que `FEATURE` est une clé de config lue une fois. Repris dans les AC de [37].

  **Fait par [34], le 04/08/2026.** L'AC 2 de [34] demandait d'énumérer tous les appelants de `gate_tree_snapshot` au moment de livrer, et c'est cette énumération qui a rendu le report absurde : la branche à pathspecs a exactement un appelant, et c'est cette protection-ci. Elle passe maintenant `:(literal)` et un `git add` par chemin. Deux choses à savoir pour qui touchera à ça : la décision de type est « un chemin, pas un glob », la même que pour les chemins gardés ([33]) ; et cette branche n'a **pas** le `|| true` de sa voisine, à dessein — un pathspec qui ne matche rien y fait tomber la fonction, donc l'appelant n'obtient aucun arbre, et c'est ce qu'il faut : un garde du tracker à qui on rendrait un arbre vide conclurait « la session n'a rien changé ». Test `a path handed to the snapshot is taken literally, not as a pattern`, avec sa réfutation.

- **Passe transversale du 06/08/2026 : cette garantie a un interrupteur depuis [13].** Le garde saute les tickets que la boucle a écrits dans la fenêtre, d'après un registre dont le chemin est **exporté** dans l'environnement de la session jugée — voir [40], sondé : une ligne appendue et l'édition n'est ni défaite ni annoncée, l'itération est verte, commitée et `resolved` avec la surface que la session s'est accordée. Le registre étant indexé par id, la portée n'est pas bornée à son propre ticket : le `Status:`, le `Failures:` et l'escalade de n'importe lequel sont atteignables. Et [42] pour les deux gardes qui ne lisent pas ce registre du tout. Tant que ces deux tickets ne sont pas livrés, la ligne « Ne pas changer le `Status:` du ticket » du tableau ne décrit pas ce qui se passe.
