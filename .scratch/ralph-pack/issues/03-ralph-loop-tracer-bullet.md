...
- **Modifié après [07] (règle de couches).** `loop_spawn` a quitté `loop.sh` pour `.claude/lib/session.sh` sous le nom `session_spawn` : la politique d'échec en avait besoin pour les sessions de re-slice, et un lib qui rappelle vers `loop.sh` inverse la pile. `loop_spawn_session` reste ici (le prompt du ticket est une affaire de boucle) et délègue le spawn. Trois entrées de `test/mutate.sh` ont été repointées vers ce fichier ; `test/layering.bats` refuse désormais l'inversion.
