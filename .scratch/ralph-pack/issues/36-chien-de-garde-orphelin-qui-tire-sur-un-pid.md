...
- **Contrainte pour [10].** Le reçu d'audit a maintenant une ligne de plus à connaître au démarrage d'un run (`N temporary director(ies) from earlier runs…`), et c'est le seul endroit du pack qui parle de ce qui vit hors du dépôt.
  ...
  **Mise à jour par [62], livré le 05/09/2026 : cette phrase a changé de texte.** Elle dit maintenant `N temporary file(s) and director(ies) from earlier runs are still in …`, parce qu'elle comptait des fichiers depuis le premier jour (`ralph-slot.writes.*`, et dix des dix-huit producteurs sont des `mktemp` sans `-d`) tout en annonçant des répertoires. La liste énumérée par ce ticket — cinq globs, six avec `ralph-slot.writes.*` — est devenue `gate_tmp_names`, dix-sept globs tenus à leur critère par un test qui dérive les producteurs des `mktemp` du pack livré. Rien à reprendre ici : c'est écrit pour qui relira cette ligne en cherchant le texte de la phrase.
