...
| Un seul run touche l'arbre de travail | **Rien.** Le verrou est par *feature*, le rollback et le commit sur vert sont par *dépôt*. Deux runs sur deux features — usage annoncé par la spec §135 et encouragé par un test — s'imputent mutuellement leurs écritures, se rollbackent l'un l'autre et peuvent s'annuler un claim. Reproduit avec deux sessions honnêtes ([22]) |
