...
| Une itération en cours finit quand un humain arrête le run | **Le trap `loop_request_stop`** ([03]) — pendant la **session**, où il est vérifié et où il tient. Pas pendant le **gate** : bash interrompt `wait` dès qu'un signal piégé arrive, le `\|\| true` avale le code > 128, et `gate_run` rend « pas de verdict » sur des branches encore vivantes. Sondé le 29/07/2026 : `Failures:` non mérité, travail rollbacké pendant que sa suite de tests tournait, branche de tests orpheline survivant au run. Le test qui couvre la promesse envoie son TERM pendant la session, avec un `TEST_CMD` qui rend la main tout de suite : il prouve le seul cas où elle tient. Propriétaire : [25] |
