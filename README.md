...
Dans un dépôt git qui a au moins un commit :

```bash
bash /chemin/vers/odysseus/init.sh --target .
```

C'est tout : `init.sh` dépose le pack (scripts, libs, substrat **déréférencé** et
épinglé), provisionne `.scratch/<feature>/issues`, `docs/adr`, `docs/playthroughs`
et `receipts`, merge un bloc dans le `CLAUDE.md` et dans le `.gitignore` sans
jamais écraser ce qui s'y trouve, écrit un `.claude/ralph.config.sh` à partir de
l'exemple, balaie ce qu'un run tué a laissé, puis rejoue tous les refus de
démarrage de `loop.sh` sur la configuration qu'il vient d'écrire. Relancé, il ne
refait que ce qui a bougé. Le moteur est du bash : `npx` (`bin/ralph-init.js`)
ne fait que lancer ce fichier, et la suite de tests installe avec `node`, `npm` et
`npx` remplacés par un échec dur.

Il **pose des questions**, et c'est la moitié de son travail. `FEATURE`,
`TEST_CMD`, `TYPECHECK_CMD`, `LANG_ARTIFACT`, `LANG_CHECK`, `SCHEDULER`,
`RUN_CMD`, `VISUAL_CMD`, `VISUAL_REAL_ASSETS` et `WORKTREE_PROVISION` n'ont pas
de défaut sûr : un `TEST_CMD="true"` passe tous les contrôles automatiques du pack et rend
le gate vert sans rien prouver, et aucun code de retour ne peut attraper ça. Hors
terminal (`--yes`, un CI), ces valeurs doivent être écrites sur la ligne de
commande — une absente est un refus, jamais un défaut.

```bash
# Non interactif, la même chose
FEATURE=ma-feature TEST_CMD="npm test" TYPECHECK_CMD="tsc --noEmit" \
  LANG_ARTIFACT=fr LANG_CHECK=on SCHEDULER=auto VISUAL_REAL_ASSETS=0 \
  RUN_CMD= VISUAL_CMD= WORKTREE_PROVISION= \
  bash /chemin/vers/odysseus/init.sh --yes --target .

$EDITOR .claude/ralph.config.sh                  # le reste se règle ici
# écrire un ticket ready-for-agent dans .scratch/ma-feature/issues/
bash .claude/loop.sh

bash /chemin/vers/odysseus/init.sh sweep --target .   # entre deux runs
```
...
Tout ça juge un arbre. Une chose est **en amont** : le `PATH`, qui décide quel `git`, quel `claude` et quel `at` la boucle exécute. Une session peut écrire dans un répertoire qui s'y trouve (`~/.local/bin`, `/usr/local/bin`, `node_modules/.bin`) : aucun ticket ne le déclare, aucun rollback ne l'atteint, et le run *suivant* — un shell frais, qui n'a rien mis en cache — l'exécute. Le pack ne fige pas son `PATH` (un projet a le droit d'avoir son `claude` où il veut) et n'efface rien hors du dépôt ; il **prend une ligne de base** de ce que ses propres noms résolvent, avant la première session, et dit sur le reçu et dans le journal ce qui a bougé sous lui. Un run qui finit en tenant un `git` qu'il n'avait pas au démarrage **n'arme aucun successeur** : la ligne mise en file porte ce `PATH` figé, et un humain doit regarder d'abord. Et une entrée de `PATH` vide ou relative — ce qu'un deux-points en trop laisse — refuse le run au démarrage, parce qu'elle ne nomme pas le même répertoire dans deux shells et qu'aucune ligne de base ne peut donc la couvrir.
...
```
init.sh                    l'installeur : le seul composant qui vive hors d'un run
bin/ralph-init.js          `npx`, qui n'est qu'un exec vers init.sh
.claude/
