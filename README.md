...
| **Failles connues** | rien ne garde `run.log`, `.run.lock/` ni les flux de session — la zone que la protection du tracker ne peut pas couvrir, parce que le flux s'y écrit pendant la fenêtre surveillée. Le tableau de [`docs/frontiere-de-confiance.md`](docs/frontiere-de-confiance.md) dit qui devrait les garder |
...
`test/mutate.sh` est le gate qui compte : il supprime une garantie à la fois et vérifie que le test censé la couvrir rougit. Treize tests de ce dépôt sont passés au vert alors que la propriété qu'ils prétendaient couvrir avait été supprimée — un test qui courait après une fenêtre de quelques microsecondes, un test qui assertait un message que la boucle imprime de toute façon, un test qui lisait le shell du développeur plutôt que le code, et deux refutations du canari qui visaient la sortie du `run` précédent au lieu de celle de la boucle. Un `VACUOUS` est un faux vert dans un pack dont le métier est de refuser les faux verts.
