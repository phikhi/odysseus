...
| **Failles connues** | une session qui pend sans émettre un token n'est bornée par rien, le filet bornant le contexte et non le temps ([23](.scratch/ralph-pack/issues/23-timeout-session.md)) · rien ne garde `run.log` ni les flux de session · ce que `.gitignore` couvre échappe au scope-guard **et** au rollback, et survit d'itération en itération ([24](.scratch/ralph-pack/issues/24-fichiers-ignores.md)) · un arrêt gracieux tombant pendant le gate vole son verdict à l'itération ([25](.scratch/ralph-pack/issues/25-arret-gracieux-pendant-le-gate.md)) · `Failures:` compte des choses qui ne sont pas des échecs et n'est jamais remis à zéro ([26](.scratch/ralph-pack/issues/26-compteur-echecs.md)) · un ticket renommé laisse deux fois le même `NN` ([27](.scratch/ralph-pack/issues/27-ticket-renomme.md)). Le tableau de [`docs/frontiere-de-confiance.md`](docs/frontiere-de-confiance.md) dit qui devrait garder quoi |
